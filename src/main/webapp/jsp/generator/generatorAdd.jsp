...
<table width="99%" border="0" cellspacing="0" cellpadding="0" id="generatorInfo">
    <tr>
        <th>生成类名</th>
        <td><input name="name" class="easyui-textbox" type="text" id="name" data-options="required:true"/></td>
    </tr>
    <tr>
        <th>描述</th>
        <td><input name="desc" class="easyui-textbox" type="text" id="desc"/>
        </td>
    </tr>
    <tr>
        <th>类路径</th>
        <td><input name="implements" class="easyui-textbox" type="text" id="implements"/>
        </td>
    </tr>
</table>
    ...
    $(function () {
        $('#saveBtn').click(function () {
            ...
            var checkExitCallBack = function checkExitCallBack(result) {
                if (result) {
                    generatorManager.add(data, function (result) {
                        if (result) {
                            if (result) {
                                alert("保存成功");
                                $('#tt').datagrid('reload');
